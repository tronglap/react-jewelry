...
const ListProduct = () => {
  ...
  // Thêm state mới để lưu trữ trạng thái của việc sắp xếp
  const [sortByPrice, setSortByPrice] = useState(false);

  // Hàm xử lý sự kiện khi người dùng chọn sắp xếp theo giá
  const handleSortByPrice = () => {
    setSortByPrice(!sortByPrice);
  };

  // Cập nhật hàm useEffect để xử lý việc sắp xếp
  useEffect(() => {
    let sortedProducts = [...productFilter];
    if (sortByPrice) {
      sortedProducts.sort((a, b) => {
        const priceA = parseFloat(a.promotion !== "" ? a.promotion : a.cost);
        const priceB = parseFloat(b.promotion !== "" ? b.promotion : b.cost);
              ...
              <Row>
                <Container>
                  <div className="sorting">
                    ...
                    <div className="soft" onClick={handleSortByPrice}>
                      {sortByPrice
